...
#ifndef SOCKETCOMM_H_INCLUDED
#define SOCKETCOMM_H_INCLUDED
...
// SocketComm : The public interface
class SocketComm
{
public:
    ...
    static SocketComm* CreateServer( unsigned int port = 2323 );
    static SocketComm* CreateClient( const char* address = "LOCALHOST", unsigned int port = 2323 );
    static void ReleaseNode( SocketComm* pNode )
        { delete pNode; }
...
protected:
    ...
    SocketComm() {}
    virtual ~SocketComm() {}
    SocketComm& operator=( const SocketComm& other );
...
public:
    ...
    TNConnection( SocketComm* pNode, TNSocketHandle hSocket, unsigned int uID )
        : m_Thread()
        , m_Socket(hSocket)
        , m_SocketMutex()
        , m_pNode(pNode)
...
// SocketCommClient : Provides the client-specific implementation (Connect, etc.)
class SocketCommClient : public SocketComm
{
public:
    SocketCommClient()
        : m_pServer()
    ...
    bool Connect( const char* address = "LOCALHOST", unsigned int port = 2323 )
        {
            bool result = false;
            ...
            if ( serverSocket != TNSocketHandle_Invalid )
            {
                ...
                {
                    assert( !"SocketCommClient::Connect : connect != 0" );
                }
            ...
            {
                assert( !"SocketCommClient::Connect : serverSocket == TNSocketHandle_Invalid" );
            }
...
}; // End : SocketCommClient


// SocketCommServer : Provides the server-specific implementation (Listen, etc.)
class SocketCommServer : public SocketComm
{
public:
    SocketCommServer()
        : m_ListenThread()
    ...
    bool Listen( unsigned int port = 2323 )
        {
            bool result = false;
            ...
            if ( m_ListenSocket != TNSocketHandle_Invalid )
            {
                ...
                if ( bindResult == 0 )
                {
                    ...
                    {
                        assert( !"SocketCommServer::Listen : listen != 0" );
                    }
                }
                else
                {
                    assert( !"SocketCommServer::Listen : bind != 0" );
                }
            ...
            {
                assert( !"SocketCommServer::Listen : m_ListenSocket == TNSocketHandle_Invalid" );
            }
...
private:
    ...
    static TNThread::RetVal TNAPI ListenThreadEntry( void* arg )
        {
            ((SocketCommServer*)arg)->ListenThread();
            TNThread::Exit();
...
}; // End : SocketCommServer



// static
inline int SocketComm::Initialize()
{
    int result = 0;
...
// static
inline void SocketComm::Finalize()
{
#if defined(TNPLATFORM_WINDOWS)
    WSACleanup();
#endif // defined(TNPLATFORM_WINDOWS)
...
// static
inline SocketComm* SocketComm::CreateServer( unsigned int port )
{
    SocketCommServer* pServer = new SocketCommServer;
...
// static
inline SocketComm* SocketComm::CreateClient( const char* address, unsigned int port )
{
    SocketCommClient* pClient = new SocketCommClient;
